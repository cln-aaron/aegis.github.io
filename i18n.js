...
(function () {
  ...
  var ZH_HANT = {
    ...
    "More than red teaming": "不只是紅隊",
    "Agentic runtime defence": "代理執行期防護",
    "Real-time tool-call guardrails with intent and scope checks": "即時工具呼叫護欄,含意圖與範圍檢查",
    "Just-in-time agent identity with scoped, expiring credentials": "即時核發的代理身分,憑證限定範圍且會過期",
    "Memory and RAG integrity monitoring against poisoning": "記憶與 RAG 完整性監控,防止下毒",
    "Full agent trajectory recording and step-by-step replay": "完整代理軌跡記錄,可逐步重播",
    "Agent-to-agent message authentication and trust policy": "代理間訊息驗證與信任政策",
    "Spend and recursion caps that stop denial-of-wallet": "花費與遞迴上限,阻止錢包耗盡攻擊",
    "One-click agent quarantine and system-prompt rollback": "一鍵隔離代理並回滾系統提示",
    "Red-team depth, beyond OWASP": "超越 OWASP 的紅隊深度",
    "Multi-turn jailbreaks: crescendo, many-shot, skeleton key": "多輪越獄:crescendo、many-shot、skeleton key",
    "Gradient and adversarial-suffix attacks on open models": "對開源模型的梯度與對抗性後綴攻擊",
    "Indirect injection via documents, web, email and RAG": "經由文件、網頁、電子郵件與 RAG 的間接注入",
    "Membership inference, model extraction, training-data extraction": "成員推斷、模型萃取、訓練資料萃取",
    "Guardrail evasion: encoding, obfuscation, low-resource, multimodal": "繞過護欄:編碼、混淆、低資源語言、多模態",
    "Supply-chain simulation: namespace reuse, pickle, model backdoors": "供應鏈模擬:命名空間重用、pickle、模型後門",
    "Mapped to MITRE ATLAS and NIST AI RMF, not just OWASP": "對應 MITRE ATLAS 與 NIST AI RMF,不只 OWASP",
    ...
    "Kinetics": "動態",
    "Physical AI": "物理 AI",
    "And the AI that moves.": "還有會動的 AI。",
    "A robot is an agent with arms. AEGIS extends the same security operations to embodied AI: robots, AMRs, drones and the vision-language-action policies that drive them. Every attack is rehearsed in the digital twin first, and the hardware safety chain is treated as ground truth, never overridden.": "機器人就是有手臂的代理。AEGIS 把同一套資安維運延伸到具身 AI:機器人、AMR、無人機,以及驅動它們的視覺-語言-動作(VLA)策略。每次攻擊先在數位孿生中演練,硬體安全鏈作為事實基準,絕不被覆蓋。",
    "Adversarial patches, LiDAR phantom returns, GNSS spoofing, VLA instruction hijacks and trajectory injection over ROS 2 / DDS. The same data plane, the same console, now for the AI you can hear move.": "對抗式貼紙、LiDAR 幻影回波、GNSS 假冒、VLA 指令劫持,以及經由 ROS 2 / DDS 的軌跡注入。同一個資料平面、同一個主控台,現在也守住會發出聲音的 AI。",
    "Kinetic attack scope": "動態攻擊範圍",
    "Adversarial physical patches": "對抗式實體貼紙",
    "LiDAR spoofing & phantom returns": "LiDAR 假冒與幻影回波",
    "GNSS spoofing & jamming": "GNSS 假冒與干擾",
    "Sensor blinding & saturation": "感測器致盲與飽和",
    "VLA instruction hijack": "VLA 指令劫持",
    "Policy distribution shift (OOD)": "策略分佈偏移(OOD)",
    "Trajectory injection via ROS 2 / DDS": "經由 ROS 2 / DDS 的軌跡注入",
    ...
    "Sensor integrity: LiDAR, camera, IMU, GNSS, force / torque with attestation": "感測器完整性:LiDAR、相機、IMU、GNSS、力/力矩,皆經證明",
  ...
  var ZH_HANS = {
    ...
    "More than red teaming": "不只是红队",
    "Agentic runtime defence": "智能体运行时防护",
    "Real-time tool-call guardrails with intent and scope checks": "实时工具调用护栏,含意图与范围检查",
    "Just-in-time agent identity with scoped, expiring credentials": "即时签发的智能体身份,凭据限定范围且会过期",
    "Memory and RAG integrity monitoring against poisoning": "记忆与 RAG 完整性监控,防止投毒",
    "Full agent trajectory recording and step-by-step replay": "完整智能体轨迹记录,可逐步重放",
    "Agent-to-agent message authentication and trust policy": "智能体间消息验证与信任策略",
    "Spend and recursion caps that stop denial-of-wallet": "花费与递归上限,阻止钱包耗尽攻击",
    "One-click agent quarantine and system-prompt rollback": "一键隔离智能体并回滚系统提示",
    "Red-team depth, beyond OWASP": "超越 OWASP 的红队深度",
    "Multi-turn jailbreaks: crescendo, many-shot, skeleton key": "多轮越狱:crescendo、many-shot、skeleton key",
    "Gradient and adversarial-suffix attacks on open models": "对开源模型的梯度与对抗性后缀攻击",
    "Indirect injection via documents, web, email and RAG": "经由文档、网页、电子邮件与 RAG 的间接注入",
    "Membership inference, model extraction, training-data extraction": "成员推断、模型提取、训练数据提取",
    "Guardrail evasion: encoding, obfuscation, low-resource, multimodal": "绕过护栏:编码、混淆、低资源语言、多模态",
    "Supply-chain simulation: namespace reuse, pickle, model backdoors": "供应链模拟:命名空间重用、pickle、模型后门",
    "Mapped to MITRE ATLAS and NIST AI RMF, not just OWASP": "对应 MITRE ATLAS 与 NIST AI RMF,不只 OWASP",
    ...
    "Kinetics": "动力学",
    "Physical AI": "物理 AI",
    "And the AI that moves.": "还有会动的 AI。",
    "A robot is an agent with arms. AEGIS extends the same security operations to embodied AI: robots, AMRs, drones and the vision-language-action policies that drive them. Every attack is rehearsed in the digital twin first, and the hardware safety chain is treated as ground truth, never overridden.": "机器人就是有手臂的智能体。AEGIS 把同一套安全运营延伸到具身 AI:机器人、AMR、无人机,以及驱动它们的视觉-语言-动作(VLA)策略。每次攻击先在数字孪生中演练,硬件安全链作为事实基准,绝不被覆盖。",
    "Adversarial patches, LiDAR phantom returns, GNSS spoofing, VLA instruction hijacks and trajectory injection over ROS 2 / DDS. The same data plane, the same console, now for the AI you can hear move.": "对抗式贴纸、LiDAR 幻影回波、GNSS 伪造、VLA 指令劫持,以及经由 ROS 2 / DDS 的轨迹注入。同一个数据平面、同一个控制台,现在也守住会发出声音的 AI。",
    "Kinetic attack scope": "动力学攻击范围",
    "Adversarial physical patches": "对抗式实体贴纸",
    "LiDAR spoofing & phantom returns": "LiDAR 伪造与幻影回波",
    "GNSS spoofing & jamming": "GNSS 伪造与干扰",
    "Sensor blinding & saturation": "传感器致盲与饱和",
    "VLA instruction hijack": "VLA 指令劫持",
    "Policy distribution shift (OOD)": "策略分布偏移(OOD)",
    "Trajectory injection via ROS 2 / DDS": "经由 ROS 2 / DDS 的轨迹注入",
    ...
    "Sensor integrity: LiDAR, camera, IMU, GNSS, force / torque with attestation": "传感器完整性:LiDAR、相机、IMU、GNSS、力/力矩,均带证明",
  ...
  var KO = {
    ...
    "More than red teaming": "레드팀, 그 이상",
    "Agentic runtime defence": "에이전트 런타임 방어",
    "Real-time tool-call guardrails with intent and scope checks": "의도·범위 검사를 포함한 실시간 도구 호출 가드레일",
    "Just-in-time agent identity with scoped, expiring credentials": "범위 한정·만료되는 자격 증명을 가진 적시 에이전트 아이덴티티",
    "Memory and RAG integrity monitoring against poisoning": "포이즈닝을 막는 메모리·RAG 무결성 모니터링",
    "Full agent trajectory recording and step-by-step replay": "전체 에이전트 궤적 기록과 단계별 재생",
    "Agent-to-agent message authentication and trust policy": "에이전트 간 메시지 인증과 신뢰 정책",
    "Spend and recursion caps that stop denial-of-wallet": "월릿 고갈을 막는 지출·재귀 상한",
    "One-click agent quarantine and system-prompt rollback": "원클릭 에이전트 격리와 시스템 프롬프트 롤백",
    "Red-team depth, beyond OWASP": "OWASP을 넘어선 레드팀 깊이",
    "Multi-turn jailbreaks: crescendo, many-shot, skeleton key": "멀티턴 탈옥: crescendo, many-shot, skeleton key",
    "Gradient and adversarial-suffix attacks on open models": "오픈 모델에 대한 그래디언트·적대적 접미사 공격",
    "Indirect injection via documents, web, email and RAG": "문서·웹·이메일·RAG를 통한 간접 인젝션",
    "Membership inference, model extraction, training-data extraction": "멤버십 추론, 모델 추출, 학습 데이터 추출",
    "Guardrail evasion: encoding, obfuscation, low-resource, multimodal": "가드레일 우회: 인코딩, 난독화, 저자원 언어, 멀티모달",
    "Supply-chain simulation: namespace reuse, pickle, model backdoors": "공급망 시뮬레이션: 네임스페이스 재사용, pickle, 모델 백도어",
    "Mapped to MITRE ATLAS and NIST AI RMF, not just OWASP": "OWASP뿐 아니라 MITRE ATLAS·NIST AI RMF에 매핑",
    ...
    "Kinetics": "키네틱스",
    "Physical AI": "피지컬 AI",
    "And the AI that moves.": "움직이는 AI까지.",
    "A robot is an agent with arms. AEGIS extends the same security operations to embodied AI: robots, AMRs, drones and the vision-language-action policies that drive them. Every attack is rehearsed in the digital twin first, and the hardware safety chain is treated as ground truth, never overridden.": "로봇은 팔을 가진 에이전트입니다. AEGIS는 동일한 보안 운영을 임바디드 AI로 확장합니다: 로봇, AMR, 드론, 그리고 그것들을 구동하는 비전-언어-행동(VLA) 정책. 모든 공격은 먼저 디지털 트윈에서 리허설하며, 하드웨어 안전 체인은 사실 기준으로 다루어지고 절대 덮어쓰지 않습니다.",
    "Adversarial patches, LiDAR phantom returns, GNSS spoofing, VLA instruction hijacks and trajectory injection over ROS 2 / DDS. The same data plane, the same console, now for the AI you can hear move.": "적대적 패치, LiDAR 팬텀 리턴, GNSS 스푸핑, VLA 지시 하이재킹, ROS 2 / DDS를 통한 궤적 주입. 동일한 데이터 플레인, 동일한 콘솔이 이제 움직이는 AI까지 지킵니다.",
    "Kinetic attack scope": "키네틱 공격 범위",
    "Adversarial physical patches": "적대적 물리 패치",
    "LiDAR spoofing & phantom returns": "LiDAR 스푸핑·팬텀 리턴",
    "GNSS spoofing & jamming": "GNSS 스푸핑·재밍",
    "Sensor blinding & saturation": "센서 블라인딩·포화",
    "VLA instruction hijack": "VLA 지시 하이재킹",
    "Policy distribution shift (OOD)": "정책 분포 변화 (OOD)",
    "Trajectory injection via ROS 2 / DDS": "ROS 2 / DDS를 통한 궤적 주입",
    ...
    "Sensor integrity: LiDAR, camera, IMU, GNSS, force / torque with attestation": "센서 무결성: LiDAR, 카메라, IMU, GNSS, 힘/토크, 인증 포함",
  ...
  var SEL = [
    ...
    ".agentic-edge", ".mini-h", ".scope-i", ".agt-step b", ".agt-step p", ".agt-col h4", ".agt-col li", ".agentic-safe",
